...
public class ServerBannedUsersActivity extends AppCompatActivity implements TeamTalkConnectionListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        emptyView.setText(R.string.msg_no_banned_users);
    ...
    private class BannedUserAdapter extends BaseAdapter {
        private final Map<String, String> banLocationCache = new HashMap<>();
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (convertView == null) {
                convertView = LayoutInflater.from(ServerBannedUsersActivity.this)
            ...
            if (u.szIPAddress != null && !u.szIPAddress.isEmpty()) {
                String loc = banLocationCache.get(u.szIPAddress);
                if (loc != null) {
                    title += " [" + loc + "]";
                } else {
                    IpGeoLocator.getLocation(u.szIPAddress, ServerBannedUsersActivity.this, result -> {
                        if (result != null) {
                            banLocationCache.put(u.szIPAddress, result);
